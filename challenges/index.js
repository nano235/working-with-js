...
const photoGallery = (title, dimensions, date) => {
  return {
    ...
    publishInfo() {
      console.log(
        `Foto "${title}" was published ${Math.floor((new Date().getTime() - date.getTime()) / 1000)} seconds ago`);
    },
  };
}
...
(for example "date") of the "photoGallery" function? */
...
Ensure that variables that will be used for destructuring of the deleted properties will not be accessible 
after the destructuring operation.
*/
